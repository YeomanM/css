...
public class GlyphRenderer
{
    ...
    public List<Point> describeWithoutNotDraw(GlyphDescription gd)
    {
        boolean draw = false;
        int endPtIndex = 0;
        int endPtOfContourIndex = -1;
        ...
        for (int i = 0; i < gd.getPointCount(); i++)
        {
            if ((gd.getFlags(i) & GlyfDescript.ON_CURVE) == 0)
            {
                continue;
            }
            ...
            points.add(new Point(gd.getXCoordinate(i), gd.getYCoordinate(i),
                    (gd.getFlags(i) & GlyfDescript.ON_CURVE) != 0, endPt));
        }
        return points;
    }
    ...
    public static class Point
    {
        ...
        public boolean like(Point other, int error) {
            if (other == null) {
                return false;
            }
